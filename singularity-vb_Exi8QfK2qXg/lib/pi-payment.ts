...
const createPaymentCallbacks = (
  ...
  const onReadyForServerCompletion = async (
    ...
    try {
      // Use the txid provided directly by the Pi SDK callback — do not
      // re-read it from the payment object, which may not be populated yet.
      await api.post(BACKEND_URLS.COMPLETE_PAYMENT(paymentId), { txid });
...
export const pay = async (options: PaymentOptions): Promise<void> => {
  // Guard: Pi SDK must be loaded and initialized (only available inside Pi Browser)
  if (typeof window === "undefined" || typeof window.Pi === "undefined" || typeof window.Pi.createPayment !== "function") {
    const sdkError = new Error(
      "Pi SDK is not available. Please open this app inside the Pi Browser to make payments."
    );
    if (options.onError) {
  ...
  try {
    // Delegates entirely to the official Pi SDK — the SDK opens its own
    // secure wallet dialog. The app never requests or handles the user's passphrase.
